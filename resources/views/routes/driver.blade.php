                                            ...
                                            @for($n=0;$n<300;$n++)
                                                @if(!empty($routes_priority[$n]))
                                                    ...
                                                    @if($routes_priority[$n]->type=='patient')
                                                        ...
                                                        @endphp
                                                        <div title="@if(isset($orders[explode(',',$routes_priority[$n]->order_id)[0]])){{date('m/d/Y', strtotime($orders[explode(',',$routes_priority[$n]->order_id)[0]]->delivery_date))}} @if(!empty(explode(',',$routes_priority[$n]->order_id)[0]->delivery_time_range) && $orders[explode(',',$routes_priority[$n]->order_id)[0]]->delivery_time_range!='9:00 AM;12:00 AM')<b>{{str_replace(':00','',str_replace(';',' - ',$orders[explode(',',$routes_priority[$n]->order_id)[0]]->delivery_time_range))}}@endif @endif" class="plus-item active-blue patient{{$routes_priority[$n]->type_id}} @if(!empty($routes_priority[$n]->special_instructions)){{'special'}}@endif">
                                                            <input type="hidden" name="order_id[]" value="{{$routes_priority[$n]->order_id}}">
                                                            <input type="hidden" name="type[]" value="{{$routes_priority[$n]->type}}">
                                                            <input type="hidden" name="type_id[]" value="{{$routes_priority[$n]->type_id}}">
                                                            <div class="eta_block">
