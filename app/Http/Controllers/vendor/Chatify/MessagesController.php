...
class MessagesController extends Controller
{
    ...
    public function idFetchData(Request $request)
    {
        ...
        User::where('id', Auth::user()->id)->update(['active_status' => DB::raw('now()')]);
        $fetch = User::where('users.id', $request['id'])->leftJoin('pharmacys','pharmacys.id','=','users.pharmacy_id')->select('users.id','users.name','users.last_name','users.phone','users.image','users.role','pharmacys.name as pharmacy_name', 'users.pharmacy_id',DB::raw('case when active_status is null or active_status < now() - interval 10 minute then 0 else 1 end as active_status'))->first();
        return Response::json([
            'favorite' => $favorite,
            'fetch' => $fetch ?? null,
            'user_avatar' => $fetch->image ?? "/images/users/default-user-image.png",
        ]);
    }
    ...
    public function getContacts(Request $request)
    {
        ...
        ->where('users.id','!=',Auth::user()->id)->leftJoin('pharmacys','pharmacys.id','=','users.pharmacy_id')
        ->select('users.id','users.name','users.last_name','users.phone','users.image','users.role','pharmacys.name as pharmacy_name', 'users.pharmacy_id',DB::raw('case when active_status is null or active_status < now() - interval 10 minute then 0 else 1 end as active_status'),DB::raw('MAX(ch_messages.created_at) max_created_at'))
    ...
    public function updateContactItem(Request $request)
    {
        User::where('id', Auth::user()->id)->update(['active_status' => DB::raw('now()')]);
        // Get user data
        $user = User::where('users.id', $request['user_id'])->leftJoin('pharmacys','pharmacys.id','=','users.pharmacy_id')->select('users.id','users.name','users.last_name','users.phone','users.image','users.role','pharmacys.name as pharmacy_name', 'users.pharmacy_id',DB::raw('case when active_status is null or active_status < now() - interval 10 minute then 0 else 1 end as active_status'))->first();
        if(!$user){
            return Response::json([
    ...
    public function getFavorites(Request $request)
    {
        ...
        foreach ($favorites->get() as $favorite) {
            // get user data
            $user = User::where('users.id', $favorite->favorite_id)->leftJoin('pharmacys','pharmacys.id','=','users.pharmacy_id')->select('users.id','users.name','users.last_name','users.phone','users.image','users.role','pharmacys.name as pharmacy_name', 'users.pharmacy_id',DB::raw('case when active_status is null or active_status < now() - interval 10 minute then 0 else 1 end as active_status'))->first();
            $favoritesList .= view('Chatify::layouts.favorite', [
                'user' => $user,
            ]);
        }
    ...
    public function search(Request $request)
    {
        ...
        $records = User::where('users.id','!=',Auth::user()->id)->leftJoin('pharmacys','pharmacys.id','=','users.pharmacy_id')->select('users.id','users.name','users.last_name','users.phone','users.image','users.role','pharmacys.name as pharmacy_name', 'users.pharmacy_id',DB::raw('case when active_status is null or active_status < now() - interval 10 minute then 0 else 1 end as active_status'))
        ->where(function($query) use ($input) {
            $query->where(DB::raw("CONCAT(users.name, ' ', last_name)"),'LIKE','%'.$input.'%')
                  ->orWhere('users.email','LIKE','%'.$input.'%')
                  ->orWhere('users.phone','LIKE','%'.$input.'%')
                  ->orWhere('users.id','=',$input);
            });
    ...
    public function setActiveStatus(Request $request)
    {
        ...
        if($activeStatus) {
            $status = User::where('id', Auth::user()->id)->update(['active_status' => DB::raw('now()')]);
        } else {
            $status = User::where('id', Auth::user()->id)->update(['active_status' => NULL]);
        }
        return Response::json([
            'status' => $status,
        ], 200);
