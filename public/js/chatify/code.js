  ...
  if (messenger != 0) {
    ...
    $.ajax({
      ...
      success: (data) => {
        ...
        messageInput.focus();
        // update info in view
        $(".messenger-infoView .info-name").text(data.fetch.name+' '+data.fetch.last_name+' #'+data.fetch.id);
        if(data.fetch.active_status>0){
          $(".messenger-infoView").find('.activeStatus').show();
          $(".m-header-messaging").find('.activeStatus').show();
          $(".messenger-list-item[data-contact=" + data.fetch.id + "]").find('.activeStatus').show();
        } else {
          $(".messenger-infoView").find('.activeStatus').hide();
          $(".m-header-messaging").find('.activeStatus').hide();
          $(".messenger-list-item[data-contact=" + data.fetch.id + "]").find('.activeStatus').hide();
        }
        if(data.fetch.role!='admin'){
          $(".messenger-infoView .info-phone").text(data.fetch.phone);
          if(data.fetch.pharmacy_id!=null){
            $(".messenger-infoView .pharmacy-name").text(data.fetch.role.charAt(0).toUpperCase() + data.fetch.role.slice(1)+': '+data.fetch.pharmacy_name+' #'+data.fetch.pharmacy_id);
          } else {
            $(".messenger-infoView .pharmacy-name").text(data.fetch.role.charAt(0).toUpperCase() + data.fetch.role.slice(1));
          }
        } else {
          $(".messenger-infoView .info-phone").text('');
          $(".messenger-infoView .pharmacy-name").text('');
        }
        $(".m-header-messaging .user-name").text(data.fetch.name+' '+data.fetch.last_name+' #'+data.fetch.id);
        // Star status
        data.favorite > 0
