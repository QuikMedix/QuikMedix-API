...
$(function () {
    ...
    if(/^\/drivers\/\d+\/users$/.test(tablePath)) {
        $('.focus-btn-group').append('<a href="'+tablePath+'/add"><button class="btn btn-primary" style="margin-left:15px;">Add New Driver</button></a>');
    }
    if(/^\/orders(?:\/\d+)?$/.test(tablePath)) {
        $('.focus-btn-group').append('<div class="btn-group dropend"><button type="button" class="btn btn-primary waves-effect waves-light dropdown-toggle" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="mdi mdi-pencil-plus"></i>Add New Order<i class="mdi mdi-chevron-right"></i></button><div class="dropdown-menu"><a href="'+tablePath+'/add" class="addorder dropdown-item">Single order</a><a class="dropdown-item" href="'+tablePath+'/facilitys_add">Facility order</a></div></div><button class="btn btn-secondary waves-effect" data-bs-toggle="offcanvas" data-bs-target="#offcanvasRight" aria-controls="offcanvasRight" style="margin-left: 10px;"><i class="mdi mdi-cloud-upload"></i> Upload Delivery Slip </button><div style="display:flex"><input style="margin-left:10px;width:100px;font-size:10px;min-height:35px;padding:0 5px" class="form-control" type="date" id="date_print"><button class="btn btn-warning" onclick="PrintDay()" style="margin-left:10px">Print Day</button></div>');
    }
