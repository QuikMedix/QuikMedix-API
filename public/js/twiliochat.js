var twiliochat = (function() {
  ...
  function fetchAccessToken(username0, handler) {
    $.post('/chat-token', {identity: username0, _token: $("input[name='_token']").val()}, null, 'json')
    ...
    $.post('/users/get', {identity: username, _token: $("input[name='_token']").val()}, null, 'json')
      .done(function(response) {
        if(response.name!='') {
    ...
    $.post('/users/get', {identity: username2, _token: $("input[name='_token']").val()}, null, 'json')
      .done(function(response) {
        if(response.name!='') {
  ...
  function initChannel(channel) {
    ...
    return tc.messagingClient.getChannelByUniqueName(channel.friendlyName);
  }
  ...
  function messageAddAjax(username0,created,body) {
    if (username0 === tc.username) {
      $.post('/chats/new_message', {chat_name: GENERAL_CHANNEL_UNIQUE_NAME,created: created,body: body,user: tc.username,_token: $("input[name='_token']").val()}, null, 'json')
      .done(function(response) {
      ...
      $.post('/chats/new_message', {chat_name: GENERAL_CHANNEL_UNIQUE_NAME,created: created,body: body,user: tc.username,not_me_author:1,_token: $("input[name='_token']").val()}, null, 'json')
      .done(function(response) {
        if(response.result!='true') {
          console.log('Failed with error: ' + response.message);
        }
  ...
  function updateChannelUI(selectedChannel) {
    ...
    if (tc.currentChannelContainer === undefined && selectedChannel.friendlyName === GENERAL_CHANNEL_NAME) {
  ...
  function addChannel(channel) {
    if (channel.friendlyName === GENERAL_CHANNEL_NAME) {
      tc.generalChannel = channel;
    }
